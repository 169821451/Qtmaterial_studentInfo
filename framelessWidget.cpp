    ...
    , ui(new Ui::framelessWidget)
{
    ...
    // 设置密码输入模式
    ui->lineEdit__Pwd->setEchoMode(QLineEdit::Password);
    ...
    // 给密码输入栏右侧添加眼睛图标（眼睛打开明文显示输入的密码）
    QAction *action = new QAction(this);
    action->setIcon(QIcon(":/img/preview-close.png"));
    connect(ui->lineEdit__Pwd,&QtMaterialAutoComplete::textEdited,this,[=]{
        static bool first = true;
        if(first)
        {
            ui->lineEdit__Pwd->addAction(action,QLineEdit::TrailingPosition);
            first = false;
        }
    ...
    connect(action,&QAction::triggered,this,[=]{
        ...
        if(stat == 0)
        {
            action->setIcon(QIcon(":/img/eyes-open.png"));
            ui->lineEdit__Pwd->addAction(action,QLineEdit::TrailingPosition);
            stat = 1;
            ui->lineEdit__Pwd->setEchoMode(QLineEdit::Normal);
        }
        else {
            action->setIcon(QIcon(":/img/preview-close.png"));
            ui->lineEdit__Pwd->addAction(action,QLineEdit::TrailingPosition);
    ...
    // 给提示控件设置固定高度
    ui->Tips_Coum->setFixedHeight(20);
...
void framelessWidget::on_btn_login_clicked()
{
    ...
    QString Pwd = ui->lineEdit__Pwd->text();
//    if(userName.isEmpty() && Pwd.isEmpty())
//    {
//        ui->Tips_Coum->textLable->setText(u8"请输入用户名");
//        ui->Tips_Coum->animationStart();
//    }
...
//    else if (Pwd.isEmpty())
//    {
//        ui->Tips_Coum->textLable->setText(u8"请输入密码");
//        ui->Tips_Coum->animationStart();
//    }
//    else
//    {
